...
<main class="site-content" role="main">
    <section class="pricing-section mt-4" aria-label="Nos formules d'abonnement">
        <div class="container">
            ...
            <div class="pricing-cards mt-4" role="list" aria-label="Liste des formules disponibles">
                ...
                <div class="pricing-card {{ $plan->is_highlighted ? 'highlighted' : '' }}"
                    ...
                    <div class="badge" role="status" aria-label="Offre spéciale">{{ $plan->highlight_text }}</div>
                    @endif

                    <h2 class="plan-title">{{ $plan->name }}</h2>
                    <p class="starting-text font-bold">{{ $plan->starting_text }}</p>
                    <p class="price" aria-label="Prix mensuel">{{ $plan->is_custom_plan ? $plan->monthly_price : $plan->monthly_price . '€/mois' }}</p>
                    <p class="see-conditions"><a href="{{ route('conditions') }}" target="_blank" rel="noopener noreferrer">*Voir conditions</a></p>
                    <p class="alt-price" aria-label="Prix annuel">
                    ...
                    <div class="btn-wrapper">
                        <button class="btn-subscribe" onclick="openForm(this)" aria-label="Souscrire à la formule {{ $plan->name }}">{{ $plan->button_text }}</button>
                    </div>
                </div>
                @empty
    ...
    <div class="modal-overlay" id="modal-overlay" role="dialog" aria-labelledby="modal-title" aria-hidden="true">
        <div class="modal-content">
            <button type="button" onclick="closeForm()" class="absolute -top-3 -right-3 bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors shadow-md" aria-label="Fermer le formulaire">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
            <h2 id="modal-title" class="text-center text-xl text-[#0099CC] font-bold mb-4">Souscrire à une offre</h2>
            <form action="{{ route('send.email') }}" method="POST" id="offerForm" aria-label="Formulaire de souscription">
                @csrf
                <div class="space-y-4">
                    <input type="text" name="name" id="input-name" placeholder="Votre nom" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0099CC] shadow-sm" aria-label="Votre nom">
                    <input type="email" name="email" placeholder="Votre email" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0099CC] shadow-sm" aria-label="Votre email">
                    ...
                    <fieldset class="radio-group bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <legend class="text-gray-700 mb-2 font-medium">Mode de paiement :</legend>
                    ...
                    <textarea name="message" placeholder="Détails supplémentaires" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0099CC] shadow-sm" aria-label="Détails supplémentaires"></textarea>
            ...
            <div id="error-message" class="hidden bg-red-500 text-white p-3 rounded mt-3" role="alert"></div>
            <div id="success-message" class="hidden bg-green-500 text-white p-3 rounded mt-3" role="alert"></div>
        </div>
    </div>
</main>
